...
namespace AccesoDatos
{
    internal class Program
    {
        ...
        static void LeerRegistrosDept()
        {
            ...
            //SI LA CADENA TIENE CARACTERES ESPECIALES, COMO POR EJEMPLO LA CONTRABARRA,
            //DEBEMOS INCLUIR UNA @ FUERA DE LA CADENA PARA QUE RECUPERE LOS LITERALES
            //SI LA CADENA TIENE PASSWORD, DEBEMOS ESCRIBIR EL PASSWORD MANUALMENTE
            ...
            //UN CURSOR NUNCA SE INSTANCIA, SOlO PODEMOS CREARLO A PARTIR DE NUESTRA CONSULTA DE SELECCION Y UN COMANDO
            SqlDataReader lector;
            //DECLARAMOS NUESTRA CONSULTA
            string sql = "select * from DEPT";
            //CONFIGURAMOS NUESTRO COMMAND
            //INDICAMOS AL COMANDO LA CONEXION A UTILZAR
            com.Connection = cn;
            //DEBEMOS INDICAR EL TIPO DE CONSULTA
            com.CommandType = System.Data.CommandType.Text;
            //INDICAMOS LA PROPIA CONSULTA
            com.CommandText = sql;
            //UNA VEZ FINALIZADO LA CONFIGURACION DEBEMOS EJECUTAR LA CONSULTA, PARA ELLO ABRIMOS LA CONEXION
            //SIEMPRE SERA ENTRAR Y SALIR
            cn.Open();
            //EJECUTAMOS LA CONSULTA MEDIANTE EL COMANDO
            //AL SER UNA CONSULTA DE SELECCION, SE UTILIZA EL METODO ExecuteReader() QUE NOS 
            //DEVUELVE UN DataReader (cursor/lector)
            lector = com.ExecuteReader();
            //PARA LEER EL READER CONTIENE UN METODO Read() QUE LEE UNA FILA Y DEVUELVE true/false
            //SI HA PODIDO LEER, CADA VEZ QUE EJECUTAMOS Read() SE MUEVE UNA FILA
            //SOLO PODEMOS IR HACIA DELANTE
            //lector.Read();
            //Read() ES BOOLEAN, POR LO QUE SI DESEAMOS RECORRER TODOS LOS REGISTROS DEBEMOS 
            //UTILIZAR UN BUCLE CONDICIONAL
            while (lector.Read())
            {
                //AQUI IREMOS RECUPERANDO COLUMNA A COLUMNA CADA DATO DE CADA FILA
                string nombre = lector["DNOMBRE"].ToString();
                string localidad = lector["LOC"].ToString();
                Console.WriteLine(nombre + " - " + localidad);
            }
            //PARA LEER LOS DATOS DE UNA COLUMNA SE UTILIZA lector["COLUMNA"], TAMBIEN PODEMOS UTILIZAR
            //lector[indice]
            //string nombre = lector["DNOMBRE"].ToString();//DNOMBRE ES DE TIPO STRING, POR LO QUE PAASMOS USAMOS ToString
        ...
        //METODO PARA ELIMINAR DEPARTAMENTO CON PARAMETROS
